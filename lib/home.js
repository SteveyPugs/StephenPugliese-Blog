...
internals.EntrySearch = function() {
	...
	var searchposts;
	connection.query("SELECT BlogID, BlogTitle, BlogContent, DATE_FORMAT(BlogCreateDate,'%W %M %D %Y') as BlogCreateDate FROM blogs WHERE BlogContent LIKE " + mysql.escape('%' + term +'%') + " ORDER BY BlogID DESC; SELECT * FROM BlogArchive; SELECT * FROM vwactivetags;", function(err, results) {
		var session = request.auth.credentials;
		if (session)
		...
		request.reply.view('search', searchposts);	
	});
}
...
internals.TagSearch = function() {
	...
	connection.query("SELECT * FROM blogtagsearch WHERE tagname LIKE " + mysql.escape('%' + tag +'%') + " ORDER BY BlogID DESC; SELECT * FROM BlogArchive; SELECT * FROM vwactivetags;", function(err, results) {
		var session = request.auth.credentials;
		if (session)
		...
		request.reply.view('archive-tag', tagposts);	
	});
...
internals.ArchiveSearch = function() {
	...
	connection.query("SELECT BlogID, BlogTitle, BlogContent, DATE_FORMAT(BlogCreateDate,'%W %M %D %Y') as BlogCreateDate FROM blogs WHERE blogdeleted <> 1 AND (DATE_FORMAT(BlogCreateDate,'%m') = " + mysql.escape(month) + " AND DATE_FORMAT(BlogCreateDate,'%Y') = " + mysql.escape(year) +  ")" + " ORDER BY BlogID DESC LIMIT 5; SELECT * FROM BlogArchive; SELECT * FROM vwactivetags;", function(err, results) {
		...
		request.reply.view('archive', archiveposts);
	});	
}
