...
internals.ArchiveSearch = function() {
	...
	connection.query("SELECT blogs.BlogID, BlogTitle, BlogContent, DATE_FORMAT(BlogCreateDate,'%W %M %D %Y') as BlogCreateDate, count(comments.CommentID) AS CommentAmount FROM blogs LEFT JOIN comments ON comments.BlogID = blogs.BlogID AND comments.CommentDeleted <> 1 WHERE blogdeleted <> 1 AND (DATE_FORMAT(BlogCreateDate,'%m') = " + mysql.escape(month) + " AND DATE_FORMAT(BlogCreateDate,'%Y') = " + mysql.escape(year) +  ")" + " GROUP BY BlogID, BlogTitle, BlogContent, BlogCreateDate ORDER BY BlogID DESC; SELECT * FROM vwArchiveList; SELECT * FROM vwAllActiveTags;", function(err, results) {
