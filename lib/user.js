...
internals.VerifyLogin = function() {
        ...
        var password = request.payload.password;
        var passwordHashed = crypto.createHash('sha256').update("blog" + password + "sp").digest("hex");
        models.Users.findAndCountAll( {where: {adminemail: email, adminpassword: passwordHashed, admindeleted: 0}, attributes: ['adminid']}).success(function(userResults) {
                if (userResults.count > 0){
                        request.auth.session.set({adminid : userResults.rows[0].dataValues.adminid})
