...
internals.changepwView = function(request, reply) {
	var session = request.auth.credentials;
	if (session){ 
		data = {userstatus:true,userid:session.adminid};
	}
	else{
		data = {userstatus:false};	
	}
    reply.view('forgot/password',data);
}

internals.changePassword = function(request, reply) {
	var userID = request.payload["id"]
	var newPW = request.payload["Password"]
	var passwordHashed = crypto.createHash('sha256').update("blog" + newPW + "sp").digest("hex");
	
	models.Users.find({where: {adminid: userID}}).success(function(user) {
		models.Users.update({adminpassword: passwordHashed},{adminid:userID}).success(function() {
			var message = {
				from: mailConfig.sendmail.from,
				to: request.auth.credentials.adminemail,
				subject: "Your password has been reset",
				html: "Your password has been successfully reset with the new password."
			};
			transport.sendMail(message, function(error){
				if (error){
					console.log(error.message)
...
exports.changepwView = internals.changepwView
exports.changePassword = internals.changePassword