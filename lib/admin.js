...
internals.CommentSave = function() {
	...
	var id = request.payload.BlogID;
		...
		connection.query("INSERT INTO comments (BlogID, CommentName, CommentContent, AdminID) VALUES (" + mysql.escape(id) + "," + mysql.escape(name) + "," + mysql.escape(comment) + "," + mysql.escape(adminid) + ")", function(err, results) {
		request.reply.redirect('/post/' + id);
		});
...
internals.Tags = function() {
	...
	connection.query("SELECT * FROM vwAllActiveTags;", function(err, results) {
		var session = request.auth.credentials;
		TagsList = {tags:results,userid:session.adminid,userstatus:true};		
		request.reply.view('tags/index', TagsList);
	});
}
...
internals.TagsSave = function() {
	var request = this;
	var session = request.auth.credentials;
	var tagname = request.payload.Tag;
	
	connection.query("INSERT INTO tags (TagName) VALUES (" + mysql.escape(tagname) + ")", function(err, results) {
		console.log(err);
